...
utils.list_wifi_networks(function(networks){
available_wifi_networks = networks;
	log.info(networks);	
	    if (available_wifi_networks && available_wifi_networks.length) {
	    conn.emit('networks', available_wifi_networks);
	    }
	    else {
		    log.error("No networks were found!");//TODO: tell client about this problem
	    }
          });
	  });
