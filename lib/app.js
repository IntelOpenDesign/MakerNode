...
function app() {
  ...
  var DashboardController = require('./dashboard_controller');
   ...
   * @method start
   * @param {} port  for the static file server and the websocket server
   * @return void
   */
  var start = function(port) {
    ...
    conf.read(APP_CONF_FILE).then(function(o) {
      app_state = o;
      utils.is_online(function(is_online) {
        if (is_online) {
          app_state.mode = 'control';
        }
      utils.get_ip_address('enp0s20f6', function(wired_ip) {
        if (wired_ip) {
          app_state.mode = 'control';
        }
        utils.stop_service('lighttpd', function() {
          servers = utils.create_servers(server_port, path.join(__dirname, '../client'));
          log.info('HTTP and WS servers listening on port', server_port);
          ...
          servers.socketio_server.on('connect', function(conn) {
            ...
            DashboardController(conn); //TODO: figure out what to do about abandoned connections
